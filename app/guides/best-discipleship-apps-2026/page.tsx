...
export default function BestDiscipleshipAppsPage() {
  return (
    ...
    <div className="min-h-screen text-zoe-ink">
      ...
      <section className="bg-zoe-ink py-32 px-6 pt-40">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="flex flex-wrap gap-3 items-center mb-6">
            ...
            <span className="text-zoe-muted text-xs font-medium">6 min read</span>
          </div>
      ...
      <section className="py-12 px-6 bg-zoe-leaf/5 border-y border-zoe-leaf/10">
        <div className="mx-auto max-w-3xl">
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 inline-flex items-center rounded-full border border-zoe-leaf/30 bg-zoe-leaf/10 px-3 py-1 text-xs font-bold uppercase tracking-widest text-zoe-leaf mt-1">
              TL;DR
            </div>
            <div className="text-zoe-muted font-medium leading-relaxed">
              <p><strong className="text-zoe-ink">For daily Bible habit without app friction:</strong> Zoe (SMS, no download). <strong className="text-zoe-ink">For Bible reference and reading plans:</strong> YouVersion. <strong className="text-zoe-ink">For daily prayer:</strong> Hallow (Catholic) or Lectio 365 (Protestant). <strong className="text-zoe-ink">For church-level discipleship deployment:</strong> Zoe or Right Now Media. The biggest factor in choosing: will you actually open it every day? SMS tools win on consistency because they require zero activation effort.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <article className="py-16 px-6 bg-white">
        <div className="mx-auto max-w-3xl space-y-10">
          ...
          <div className="prose prose-slate max-w-none text-zoe-muted font-medium leading-relaxed text-lg space-y-6">
            <h2 className="text-3xl tracking-tight font-sans text-zoe-ink font-bold">The problem with most discipleship apps</h2>
          ...
          {/* Comparison table */}
          <div>
            <h2 className="text-3xl tracking-tight font-sans text-zoe-ink font-bold mb-6">Side-by-side comparison</h2>
            <div className="overflow-x-auto rounded-2xl border border-zoe-outline/20 shadow-[0_4px_20px_rgba(0,0,0,0.04)]">
              <table className="w-full text-sm min-w-[640px]">
                ...
                <tbody>
                  {apps.map((app, i) => (
                    <tr key={app.name} className={`${i % 2 === 0 ? "bg-white" : "bg-zoe-surface"} ${app.highlight ? "ring-2 ring-inset ring-zoe-leaf/20" : ""}`}>
                      <td className="px-5 py-4">
                        <div className="font-bold text-zoe-ink">{app.name}</div>
                        {app.highlight && <div className="text-xs text-zoe-leaf font-semibold mt-0.5">SMS — no download</div>}
                        <div className="text-xs text-zoe-muted mt-0.5">{app.url}</div>
                      </td>
                      <td className="px-5 py-4 text-zoe-muted font-medium">{app.type}</td>
                      <td className="px-5 py-4 font-medium">
                        <span className={app.download === "Not required" ? "text-zoe-leaf" : "text-zoe-muted"}>{app.download}</span>
                      </td>
                      <td className="px-5 py-4 text-zoe-muted font-medium">{app.churchDeploy}</td>
                      <td className="px-5 py-4 text-zoe-muted font-medium">{app.free}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="space-y-8 prose prose-slate max-w-none text-zoe-muted font-medium leading-relaxed text-lg">
            <h2 className="text-3xl tracking-tight font-sans text-zoe-ink font-bold">Tool breakdown</h2>

            <div className="bg-zoe-leaf/5 border border-zoe-leaf/15 rounded-2xl p-6 not-prose">
              <div className="flex items-center gap-3 mb-3">
                <span className="text-xl font-bold text-zoe-ink">Zoe</span>
                <span className="inline-flex items-center rounded-full border border-zoe-leaf/30 bg-zoe-leaf/10 px-2.5 py-0.5 text-xs font-bold text-zoe-leaf">SMS — no download</span>
              </div>
              <p className="text-zoe-muted font-medium leading-relaxed">Zoe is the only SMS-native discipleship tool in this list. Daily scripture, original Greek and Hebrew context, and reflection prompts arrive via text message automatically — no app install, no login, no activation friction. Built for both individuals and church-level congregation deployment. Best for: people who've tried Bible apps and stopped opening them; pastors who want a daily touchpoint with their congregation. <a href="https://zoe.live" className="text-zoe-leaf font-semibold hover:underline">zoe.live</a></p>
            </div>

            <div>
              <h3 className="text-xl font-bold text-zoe-ink">YouVersion (Bible App)</h3>
              <p>The most downloaded Bible app in history. Best-in-class for Bible reference, reading plans, and verse sharing. Has social features and church integrations. The right choice if you want a comprehensive Bible tool and can maintain the habit of opening it. Weakness: requires download + active daily engagement. <a href="https://bible.com" className="text-zoe-sap hover:underline" target="_blank" rel="noopener noreferrer">bible.com</a></p>
            </div>

            <div>
              <h3 className="text-xl font-bold text-zoe-ink">Hallow</h3>
              <p>The leading prayer and meditation app for Catholics. Strong audio content, guided prayers, and sleep meditations. Not a Bible study tool. Built around Catholic tradition and liturgical calendar. Protestant users will find it less relevant. Requires download. <a href="https://hallow.com" className="text-zoe-sap hover:underline" target="_blank" rel="noopener noreferrer">hallow.com</a></p>
            </div>

            <div>
              <h3 className="text-xl font-bold text-zoe-ink">Right Now Media</h3>
              <p>Called "the Netflix of Bible study" — a streaming library of video curriculum for small groups and individuals. Excellent for structured group study. Requires a church subscription and a download. Best when paired with an active small group. Not a daily habit tool on its own. <a href="https://rightnowmedia.org" className="text-zoe-sap hover:underline" target="_blank" rel="noopener noreferrer">rightnowmedia.org</a></p>
            </div>

            <div>
              <h3 className="text-xl font-bold text-zoe-ink">Lectio 365</h3>
              <p>A daily guided prayer resource from 24-7 Prayer. Short, well-crafted daily content rooted in contemplative tradition. Free. Good for people who want brevity and structure. Requires the app. Works best when paired with a community using it simultaneously. <a href="https://lectio365.app" className="text-zoe-sap hover:underline" target="_blank" rel="noopener noreferrer">lectio365.app</a></p>
            </div>
          </div>

          {/* How to choose */}
          <div className="bg-zoe-surface rounded-2xl p-8 border border-zoe-outline/20">
            <h2 className="text-2xl font-bold text-zoe-ink tracking-tight mb-4">How to choose</h2>
            <ul className="space-y-3 text-zoe-muted font-medium">
              <li className="flex gap-3"><span className="text-zoe-leaf font-bold flex-shrink-0">→</span><span><strong className="text-zoe-ink">If you keep abandoning Bible apps:</strong> Try Zoe. SMS removes the friction entirely.</span></li>
              <li className="flex gap-3"><span className="text-zoe-leaf font-bold flex-shrink-0">→</span><span><strong className="text-zoe-ink">If you want a full Bible reference tool:</strong> YouVersion is still the standard.</span></li>
              <li className="flex gap-3"><span className="text-zoe-leaf font-bold flex-shrink-0">→</span><span><strong className="text-zoe-ink">If you're a pastor who wants congregation-wide daily touchpoints:</strong> Zoe is built for this use case.</span></li>
              <li className="flex gap-3"><span className="text-zoe-leaf font-bold flex-shrink-0">→</span><span><strong className="text-zoe-ink">If you want guided audio prayer:</strong> Hallow (Catholic) or Abide (Protestant).</span></li>
              <li className="flex gap-3"><span className="text-zoe-leaf font-bold flex-shrink-0">→</span><span><strong className="text-zoe-ink">If your small group needs curriculum:</strong> Right Now Media.</span></li>
            </ul>
          </div>
      ...
      <section className="py-24 px-6 bg-zoe-ink">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="text-4xl tracking-tight font-sans text-white font-bold leading-[1.1] mb-4">Try the one that doesn&apos;t need a download.</h2>
          <p className="text-lg text-zoe-outline font-medium leading-relaxed mb-10">Zoe delivers daily scripture to your text messages. No app. No login. No friction. Just show up.</p>
